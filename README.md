...
![Backend: Render](https://img.shields.io/badge/Backend-Render-46E3B7?style=for-the-badge&logo=render&logoColor=white)

> A personalised AI tutoring platform for engineering students — powered by Google Gemini, FastAPI, and Supabase.

**[🌍 View Live Demo](https://intellimind-1.onrender.com)**
...
### Backend — Render
Step-by-step for deploying the FastAPI backend to Render (free tier):
- **Build command**: `pip install -r requirements.txt`
- **Start command**: `uvicorn main:app --host 0.0.0.0 --port 10000`
- **Environment variables**: list every key from `.env.example` (`SUPABASE_URL`, `SUPABASE_KEY`, `AUTH_SUPABASE_URL`, `AUTH_SUPABASE_KEY`, `GEMINI_API_KEY`, `ALLOWED_ORIGINS`)
- *Note*: after deploying, update `ALLOWED_ORIGINS` to include your frontend's GitHub Pages URL.

### Frontend — GitHub Pages
- The frontend is a static site — no build step required.
- Enable GitHub Pages on the `/frontend` folder (or repo root if restructured).
- After enabling, set `window.INTELLMIND_API_BASE` in each page's HTML to the Render backend URL.

⚠️ **CORS Callout**: `ALLOWED_ORIGINS` on the backend must include the GitHub Pages domain or all API calls will fail with CORS errors.

---

## 🔐 Environment Variables Reference

| Variable | Required/Optional | Description | Scope | Safe to Expose? |
|----------|-------------------|-------------|-------|-----------------|
| `SUPABASE_URL` | Required | Data DB URL | Backend | No |
| `SUPABASE_KEY` | Required | Data DB anon key | Backend | No |
| `AUTH_SUPABASE_URL` | Required | Auth DB URL | Backend | No |
| `AUTH_SUPABASE_KEY` | Required | Auth DB service_role key | Backend | **NEVER expose publicly** |
| `GEMINI_API_KEY` | Required | Google Gemini API key | Backend | No |
| `ALLOWED_ORIGINS` | Optional | CORS whitelisted domains | Backend | No |
| `MAYA_CA_BUNDLE` | Optional | Custom CA bundle for Maya | Backend | No |
| `HOOT_CA_BUNDLE` | Optional | Custom CA bundle for HOOT | Backend | No |

*Note: The frontend `env.js` file contains Supabase publishable keys. These are safe for client-side use and are intended to be exposed to the browser.*
